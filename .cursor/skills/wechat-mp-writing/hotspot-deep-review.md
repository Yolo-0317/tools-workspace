...
| 形态 | **纯段落**，禁止 `> ` / `##` / 「一、二、三」小标题 |
| 长度 | prompt ≥2000 字；**门禁 ≥1920**（默认容差 80），6～9 段 |
...
| 找图顺序 | 先抓同题公开报道页 og:image / 正文图；报道图能凑齐时不生成 |
| 生成兜底 | 报道图不足 4 张时，**只补缺口**生成原创新闻插画。画面必须含事件主体、地点/处置线索，禁止凭空添加人物身份、伤亡数字、机构标识或现场细节 |
...
- 每段 **2～4 句、≤130 字**（`reflow_hotspot_layout`）  
