...
- 作者题名为“唐·柳宗元《江雪》”。
- 主诗句使用 `STXingkaiSC-Bold` 和实测暖米白 `#E4D7C0`；栏目签与作者题名使用 `STKaitiSC-Regular` 和实测暖灰米白，不使用红色标题或深色描边。
...
- 主诗句使用源图实测暖米白 `(228/255, 215/255, 192/255)` 和 `STXingkaiSC-Bold`；小字使用 `STKaitiSC-Regular` 及各自实测暖灰米白；全部移除深色描边，只保留黑色柔和阴影。
