...
class CyberDivinationEpisodeManifestTests(unittest.TestCase):
    def test_builds_the_fixed_timeline(self):
        ...
        self.assertEqual(manifest.lines[0].revision, 2)
        self.assertEqual(manifest.lines[4].revision, 2)
        self.assertEqual(
            [(line.id, line.role, line.text, line.start_ms) for line in manifest.lines],
            [
                ("01-ayan-question", "ayan", "想吃", 0),
                ...
                ("06-zhixia-reveal", "zhixia", "你都吃了多少啦？", 10100),
    ...
    def test_overlay_plan_separates_classic_text_from_comedy(self):
        ...
        self.assertEqual(cards[0]["text"], "想吃")
        self.assertEqual(cards[2]["text"], "颐，贞吉。观颐，自求口实。")
        self.assertEqual(cards[3]["text"], "卦说能吃？")
        self.assertEqual(cards[4]["text"], "你都吃了多少啦？")
        self.assertEqual(cards[5]["text"], "前八块……是试吃。")
        self.assertEqual(
