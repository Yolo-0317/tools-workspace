#!/usr/bin/env python3
"""Run five-day ranking V3 in two explicit manual stages."""
...
def dispatch_command(args: argparse.Namespace) -> Path:
    if args.command == "diagnose-train":
        ...
        return write_five_day_ranking_v3_train(
            review,
            args.output_dir,
        )
    ...
    if (
        train.payload["status"] == "POLICY_SET_DEGENERATE"
        or not train.validation_eligible
        or train.winner_policy_id is None
        or train.winner_policy_hash is None
    ):
        raise ValueError("ranking v3 train artifact has no winner")
    ...
    validation_path = (
        ...
        / f"ranking-v3-validation-{trial_identity}.json"
    )
    ...
    if (
        ...
        raise ValueError("ranking v3 parent lineage mismatch")
    review = build_five_day_ranking_v3_validation_review(
        research,
        train,
        parent_research_identity=parent_identity,
    )
    return write_five_day_ranking_v3_validation(
        review,
        args.output_dir,
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        print("五日排名V3诊断失败", file=sys.stderr)
        return 2
    print(path)
    return 0
