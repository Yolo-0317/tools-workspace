...
| 能力 | 用途 |
|------|------|
| `HARRYPUTTER_MAX_PUBLIC_CHAPTER=1` | 公网试读 |
...
> **原则**：8883 映射的是整台 Caddy Hub（SideStore / Home Hub / English Buddy / HarryPutter 等），**开试读 = 扩大整站攻击面**，不能只做 harryputter 半套。

### 4.1 必做（推广前）

- [ ] **admin 强密码** — `harryputter/.env` 中 `HARRYPUTTER_ADMIN_PASSWORD` 非弱口令；`.env` 已在 `.gitignore`
...
- [ ] **日志与告警** — 至少定期看 `harryputter/logs/`、Caddy/docker 日志；异常流量能发现
- [ ] **一键下线** — 熟悉「Caddy 注释 harryputter 块 + `docker compose restart caddy`」或停 launchd harryputter

### 4.2 强烈建议

- [ ] **登录限速** — `/api/auth/login` 按 IP 限流（防 admin 暴力破解）
- [ ] **Caddy 层 rate limit** — 对 `/harryputter/*` 限制单 IP 请求率（防刷带宽、扫目录）
...
| 风险 | 说明 | 缓解 |
|------|------|------|
| 8883 共享 Hub | 攻击者扫的是整站，不只 harryputter | 限流 + 各服务鉴权 + 监控 |
...
- 导入 Skill：`.cursor/skills/harryputter-import/`
