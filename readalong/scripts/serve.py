...
MAX_PUBLIC_CHAPTER = int(os.environ.get("READALONG_MAX_PUBLIC_CHAPTER", "1"))
FULL_ACCESS_PREFIXES = tuple(
    p.strip()
    for p in os.environ.get(
        "READALONG_FULL_ACCESS_CIDRS",
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path.rstrip("/") == "/api/health":
            self._send_json({"ok": True, "service": "readalong"})
            return
        ...
        if path == "/output/catalog.json":
            self._send_json(self._filtered_catalog())
            return
        if self._path_restricted(path):
            self._send_json(
                {
    ...
    def do_POST(self) -> None:
        ...
        if path.rstrip("/") == "/api/auth/login":
            ...
            if not verify_login(username, password):
                ...
                return
            self._send_json(
                {"ok": True, "username": username, "loggedIn": True},
                extra_headers=[("Set-Cookie", set_session_cookie_header(username))],
            )
            return
...
def main() -> None:
    os.chdir(ROOT)
    if not auth_enabled():
        print("WARN: READALONG_ADMIN_PASSWORD unset — login disabled", flush=True)
    server = ThreadingHTTPServer((HOST, PORT), Handler)
    mode = "full" if MAX_PUBLIC_CHAPTER <= 0 else f"demo≤ch{MAX_PUBLIC_CHAPTER}+login"
    print(f"readalong http://{HOST}:{PORT}/web/ ({mode})", flush=True)
