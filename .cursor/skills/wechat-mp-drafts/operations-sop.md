...
| **成稿 LLM** | 定时稿只用 Codex CLI；用户主动的热点深评与文学/典籍稿使用固定 DeepSeek Chrome 会话的双确认流程；任一路线失败都禁止回退 |
...
| `literary` | 典籍节目与文学类；固定 DeepSeek 会话写稿，独立槽位，必须两次确认 |

### DeepSeek 浏览器双确认

1. Agent 完成选题、三个来源域以上的事实包和提示词，先给用户确认。
2. 用户确认后，OpenCLI 绑定当前 Chrome 的固定“公众号爆文秘诀”会话；只提取本轮新增回复。
3. Agent 核事实、编辑、高亮并寻找公开来源配图，展示推送前报告。
4. 用户第二次确认后才允许调用微信草稿 API。

若 DeepSeek 退出登录，提示：“DeepSeek 当前未登录。请在 Chrome 登录 DeepSeek，打开固定的‘公众号爆文秘诀’会话并保持为当前标签，然后回复‘已登录’。”登录恢复只回到已确认提示词状态，不重复消耗第一次确认，也不绕过第二次确认。
