...
| 点时参考数据显式刷新 | `PYTHONPATH=stock-ai:a-share-short-term-trading stock-ai/.venv/bin/python stock-ai/scripts/sync/sync_buy_point_reference_data.py --start 2024-01-02 --end latest` |
...
买点优先入口没有定时安装项。旧四轨仍可由现有调度产出研究记录，但在 V1.3 中只作为影子漏选对照，不能提供正式买入资格。
