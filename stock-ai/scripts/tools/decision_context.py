...
## 分析输出纪律（必须遵守）
1. 所有操作建议必须先符合「投顾主策略」阶段与目标组合，再符合执行卡价位与红线。
2. 若技术信号或用户口头意图与投顾阶段/红线冲突，必须**明确拒绝**并给出替代方案。
3. 新开仓 / 加仓须给出：**试探股数（100 整数倍）**、**止损价**、**预估账户风险占比**、**是否符合盈亏比≥2:1**。
4. 全中文，决策支持非投资建议；禁止绝对买卖指令。
5. 个股存在 3—5 交易日有效决策周期时，普通日内波动只能补充观察；只有价格触发、实际仓位变化、趋势结构实变、公司重大事件、市场或板块反转五类硬事件可以提前改变动作。
6. 决策记忆中的「锁定动作」由确定性状态机生成，AI 不得覆盖或提出相反动作。
"""
...
def inject_decision_context(
    ...
    *,
    ...
    parts = [
        prompt.rstrip(),
        "",
        "## 必须遵循的决策上下文（优先级：投顾主策略 > 执行卡价位 > 通用策略）",
        ctx,
    ]
