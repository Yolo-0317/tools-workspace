...
| 外网 | https://hub.yoloworld.site:8883/readalong/web/（**试读第 1 章**；内网 IP 全章） |

**试读门禁**：外网默认仅第 1 章；内网 IP 白名单全章。**管理员登录**（外网全章）：右上角「登录」，`admin` 账号密码见本机 `readalong/.env`（勿提交 git）。
...
**导入新章**：见 `.cursor/skills/readalong-import/`（须指定书目，如 `BOOK=hp01 ./scripts/pipeline.sh 11` = 第一部第十一章）。
